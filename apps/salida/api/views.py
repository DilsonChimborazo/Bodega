...
class SalidaViewSet(ViewSet):
    ...
    def retrieve(self, request, pk=int):
        serializer = SalidaSerializers(Salida.objects.get(pk=pk))
        return Response(status=status.HTTP_200_OK, data=serializer.data)
    ...
    def update(self, request, pk):
        try:
            ...
            serializer = SalidaSerializers(serializer, data=request.data)
    ...
    def partial_update(self, request, pk):
        try:
            serializer = Salida.objects.get(pk=pk)
            serializer = SalidaSerializers(serializer, data=request.data, partial=True)
            serializer.is_valid(raise_exception=True) 
            serializer.save()
            return Response(status=status.HTTP_200_OK, data=serializer.data)
        except Salida.DoesNotExist:
